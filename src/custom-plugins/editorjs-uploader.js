...
export default class Uploader {
  ...
  async uploadByFile(file, { onPreview }) {
    ...
    const upload = await ajax.post({
      url: this.config.uploader.addTokenToURL(
        `${this.config.uploader.baseURL}files`
      ),
      data: formData,
    ...
    const response = {
      ...
      file: {
        ...
        fileId: result.id,
        fileURL: this.config.uploader.baseURL + "files/" + result.id,
    ...
    if (this.getCurrentFile) {
      ...
      if (currentPreview) {
        this.config.uploader.setCurrentPreview(
          this.config.uploader.addTokenToURL(currentPreview)
        );
      }
    }
